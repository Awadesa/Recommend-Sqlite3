...
# ===============================
# 5) المنتجات الأعلى تقييماً
# ===============================
def best_rated(top_n: int = 5):
    products = get_products()
    for p in products:
        p["product_rating"] = float(p.get("product_rating", 0))
    # ترتيب المنتجات حسب التقييم (من الأعلى للأقل)
    products.sort(key=lambda x: x["product_rating"], reverse=True)
